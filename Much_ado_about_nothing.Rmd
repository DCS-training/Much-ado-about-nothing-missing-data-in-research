...
vis_rescue_imp <- ggplot(pmm_rescue_complete, aes(x = Sepal.Width, y = Petal.Length, color = Petal.Length_NA)) +
  geom_point()+
  geom_smooth(method = "lm") +
  geom_miss_point() + # using grid_miss_point() from earlier to view difference
  theme(legend.position = "bottom") +
  labs(title = title = "MNAR missing values with auxillary variatble (Petal.Width)", 
       subtitle =  "Petal.Length imputed via pmm")

grid.arrange(vis_rescue_imp, vis_pmm_imp) # Using grid.arrange() to compare plots, lets see the difference the auxillary MNAR impute vs no auxillary MNAR impute
