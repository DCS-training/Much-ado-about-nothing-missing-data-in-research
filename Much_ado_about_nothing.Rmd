...
# Step 2- logistic regression to test missing data hypothesis + data vis
 ...
 model_log<- glm(air_temp_c_NA ~ sea_temp_c, family="binomial", data = Ocean_shadow )
...
Under all 3 conditions of missing data, we will artificially remove 30% of their data. This will be achieved using the `missMethods` package.  
...
vis_lm_imp <- ggplot(lm_imp_MCAR, aes(x = Petal.Width, y = Petal.Length)) +
  geom_smooth(method = "lm", formula = y ~ x, color = "black", alpha = .5) +
  geom_point(aes(color = Petal.Length_NA), alpha = .7)+
  theme(legend.position = "bottom") +
...
For this we will use the predictive mean matching (pmm) within a multiple imputed chained equation setup - Again, this is all very fancy speak, which we will cover in detail next week. But for today, it is good to know that this is one of the recommended best practice approaches.
...
vis_lm_imp <- ggplot(lm_imp_MAR, aes(x = Petal.Width, y = Petal.Length)) +
    geom_smooth(method = "lm", formula = y ~ x, color = "black", alpha = .5) +
  geom_point(aes(color = Petal.Length_NA), alpha =.7)+
  theme(legend.position = "bottom") +
...
To simulate MNAR data in the IRIS dataset, we will need to remove the `Petal.Width` variable, due to it's high covariance with `Petal.Length`. So lets imagine that the research in this case forgot to record data on `Petal.Width`, `Sepal.Width`, and `Species`... and then realised entries were missing. Let's examine the effect this would have, and what they could do about it

```{r, error = FALSE, warning = FALSE}
iris_id <- iris %>% mutate(ID = row_number()) # Creating ID number - this will be useful for later!
MNAR_iris <- iris_id %>% select(Petal.Length, Sepal.Length, ID) %>% # Choosing only Petal.Length + Sepal.Length - other variables "disapeered"
  delete_MNAR_censoring( cols_mis = "Petal.Length",  p =.3, where = "both") %>% bind_shadow()# here we have asked data to be missing in the `Petal.Length` variable, and for 30% of data to be missing in the MNAR condition.  Setting `where` to both, so missing data present at both sides.

MNAR_summary <- MNAR_iris %>% 
  dplyr::select(`Sepal.Length`,  `Petal.Length`) %>% # selecting to avoid summaries of shadow variables.
  tbl_summary( statistic = list(
      all_continuous() ~ "{mean} ({sd})",
      all_categorical() ~ "{n} / {N} ({p}%)"
    ))
...
vis_MNAR <- ggplot(MNAR_iris, aes(x = Sepal.Length, y = Petal.Length, color = Petal.Length_NA)) + # Using Sepal.Width  to examine here
    geom_smooth(method = "lm", formula = y ~ x, color = "black", alpha = .5) +
  geom_point(alpha = .7)+
  geom_miss_point() + # using grid_miss_point() from earlier to view difference
  theme(legend.position = "bottom") +
...
# Re-setting comparison plots of MAR and MCAR to match data
vis_MAR <- ggplot(MAR_iris, aes(x = Sepal.Length, y = Petal.Length, color = Petal.Length_NA)) +
    geom_smooth(method = "lm", formula = y ~ x, color = "black", alpha = .5) +
  geom_point(alpha = .7)+
  geom_miss_point() +
  theme(legend.position = "bottom") +
...
vis_MCAR <- ggplot(MCAR_iris, aes(x = Sepal.Length, y = Petal.Length, color = Petal.Length_NA)) +
    geom_smooth(method = "lm", formula = y ~ x, color = "black", alpha = .5) +
  geom_point(alpha = .7)+
  geom_miss_point() + # using grid_miss_point() from earlier to view difference
  theme(legend.position = "bottom") +
...
vis_complete <- ggplot(iris, aes(x = Sepal.Length, y = Petal.Length)) +
    geom_smooth(method = "lm", color = "black", alpha = .5) + 
  geom_point(aes(color = Species), alpha = .7)+
  theme(legend.position = "bottom") +
  labs(title = "Complete original data - no missing values") # Plot for comparison with original data.
...
model_complete <- lm(Petal.Length ~ Sepal.Length, iris)# New original to compare more accurately

Original_model <- tbl_regression(model_complete) %>% 
  modify_column_unhide(column = std.error) %>% 
  add_glance_table(include = c(r.squared, adj.r.squared, 
                               statistic, df.residual, p.value))

model_MNAR <- lm(Petal.Length ~ Sepal.Length, MNAR_iris) # Establishing model for MNAR

model_MAR <- lm(Petal.Length ~ Sepal.Length, MAR_iris) # Model for MAR on Sepal Length

model_MCAR <- lm(Petal.Length ~ Sepal.Length, MCAR_iris) # Model for MCAR on Sepal Length
...
vis_MNAR_delete <- ggplot(delete_MNAR, aes(x = Sepal.Length, y = Petal.Length)) +
    geom_smooth(method = "lm", formula = y ~ x, color = "black", alpha = .5) +
  geom_point(aes(color = Petal.Length_NA), alpha =.7)+
  theme(legend.position = "bottom") +
  labs(title = "MNAR missing values at Petal.Length")
...
model_MNAR_del <- lm(Petal.Length ~ Sepal.Length, delete_MNAR) # Establishing model
...
vis_mean_imp <- ggplot(mean_imp_MNAR, aes(x = Sepal.Length, y = Petal.Length)) +
  geom_smooth(method = "lm", formula = y ~ x, color = "black", alpha = .5) +
  geom_point(aes(color = Petal.Length_NA), alpha =.7)+
  theme(legend.position = "bottom") +
...
model_MNAR_imp <- lm(Petal.Length ~ Sepal.Length, mean_imp_MNAR) # Establishing model
...
```{r}
# Here we use the impute_lm() function to impute via regression
lm_imp_MNAR <- MNAR_iris %>% impute_lm(Petal.Length ~ Sepal.Length)
...
vis_lm_imp <- ggplot(lm_imp_MNAR, aes(x = Sepal.Length, y = Petal.Length)) +
  geom_smooth(method = "lm", formula = y ~ x, color = "black", alpha = .5) +
  geom_point(aes(color = Petal.Length_NA), alpha =.7)+
  theme(legend.position = "bottom") +
...
model_MNAR_lm_imp <- lm(Petal.Length ~ Sepal.Length, lm_imp_MNAR) # Establishing model
...
vis_pmm_imp <- ggplot(pmm_MNAR_complete, aes(x = Sepal.Length, y = Petal.Length)) +
  geom_smooth(method = "lm", formula = y ~ x, color = "black", alpha = .5) +
  geom_point(aes(color = Petal.Length_NA), alpha = .7)+
  theme(legend.position = "bottom") +
...
model_MNAR_pmm_imp <- lm(Petal.Length ~ Sepal.Length, pmm_MNAR_complete) # Establishing model
...
```{r, message= FALSE, warning= FALSE, results='hide'}
petal_width_id <- iris %>% select(Petal.Width, Sepal.Width, Species) %>% mutate(ID = row_number()) 
...
vis_rescue_imp <- ggplot(pmm_rescue_complete, aes(x = Sepal.Length, y = Petal.Length)) +
  geom_smooth(method = "lm", color = "black", alpha = .5) + 
  geom_point(aes(color = Petal.Length_NA), alpha = .7)+
  theme(legend.position = "bottom") +
  labs(title = "MNAR missing values with auxillary variables", 
       subtitle =  "Petal.Length imputed via pmm")
...
model_rescue_pmm_imp <- lm(Petal.Length ~ Sepal.Length, pmm_rescue_complete) # Establishing model


model_complete <- lm(Petal.Length ~ Sepal.Length, iris)# New original to compare more accurately

original_data_model <- tbl_regression(model_complete) %>% 
  modify_column_unhide(column = std.error) %>% 
  add_glance_table(include = c(r.squared, adj.r.squared, 
                               statistic, df.residual, p.value))
...
tbl_merge(tbls = list(original_data_model, MNAR, MNAR_pmm_imp, MNAR_rescue_imp ),
          tab_spanner = c("Original", "MNAR", "MNAR predictive mean matching imputation", "Rescued MNAR PMM")) 

```

I hope this helps to highlight the importance of auxillary/additional data. It may also inspire discussions/debates/arguments with your ethics panel about defining the collection of "essential data only". To help, make sure in your research design that you can theoretically justify why your auxillary variables can be useful for mitigating missing data.  
